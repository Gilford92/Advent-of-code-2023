...
int cube_conundrum::part_one(const std::string& file_name, bool debug)
{
	...
	for(std::string line; std::getline(input_file, line);)
	{
		...
		parse(line, debug, game_id, red, green, blue);
...
int cube_conundrum::part_two(const std::string& file_name, bool debug)
{
	int powers_sum = 0;
	...
	for(std::string line; std::getline(input_file, line);)
	{
		...
		if(debug)
		{
			std::cout << "Added power: " << power << " (" << powers_sum << ")\n";
		}
	}
...
void cube_conundrum::parse(std::string line, const bool debug, int& game_id, int& red, int& green, int& blue)
{
	if(debug)
	{
